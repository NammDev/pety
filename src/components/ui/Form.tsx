...
export const FieldError: FC<FieldErrorProps> = ({ name }) => {
  ...
  return <div className='mt-2 text-sm font-bold text-red-500'>{error.message as string}</div>
}
  ...
  return (
    <FormProvider {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <fieldset className={cn('flex flex-col', className)} disabled={form.formState.isSubmitting}>
          {children}
        </fieldset>
      </form>
    </FormProvider>
